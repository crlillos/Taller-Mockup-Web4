...
def login():
    if request.method == 'POST':
        ...
        session['rol'] = usuario.get('Rol', 'cliente').lower()
...
def admin_dashboard():
    ...
    for cita in citas_ref:
        data = cita.to_dict()
        data['id'] = cita.id
        todas_las_citas.append(data)

    if buscar:
        todas_las_citas = [
            c for c in todas_las_citas
            if buscar in c.get("cliente", "").lower()
            or buscar in c.get("telefono", "").lower()
            or buscar in c.get("servicio", "").lower()
            or buscar in c.get("fecha", "").lower()
        ]

    total = len(todas_las_citas)
    inicio = (page - 1) * por_pagina
    fin = inicio + por_pagina
    citas_paginadas = todas_las_citas[inicio:fin]
...
@app.get("/confirmar_eliminar/<string:id>")
def confirmar_eliminar(id):
    cita_ref = db.collection('citas').document(id).get()
    cita = cita_ref.to_dict()
    cita['id'] = id
    return render_template("confirmar_eliminar.html", cita=cita)
...
@app.post("/actualizar_estado/<string:id>")
